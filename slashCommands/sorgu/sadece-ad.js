...
module.exports = {
    name: "sadece-ad", //the command name for the Slash Command
    description: "Sadece Ad Sorgu", //the command description for Slash Command Overview
    cooldown: 1.5,
    ...
    run: async (client, interaction) => {
        ...
        try {
           ...
           // interaction.reply({ content: "Yükleniyor...", ephemeral: true });
            ...
            var ilçe = interaction.options.getString("ilçe")

            //
            interaction.reply({ content: "DM Üzerinden Gönderildi , Eğer DM Kutusunda Birşey Yoksa Bu Biraz Zaman Alabilir Sabırla Bekleyin", ephemeral: true })
            ...
            if (il) {
                ...
                con.query(`SELECT * FROM 101m WHERE ADI="${isim}" AND NUFUSIL="${il}" AND NUFUSILCE="${ilçe}"`, function (err, result) {
                    if (err) throw err;
              ...
              let as31 = data.map((o) => `TCSİ: ${o.TC} | ADI: ${o.ADI} | DOGUMTARİHİ: ${o.DOGUMTARIHI} | İL: ${o.NUFUSIL} | İLÇE: ${o.NUFUSILCE} | ANNE ADI: ${o.ANNEADI} | ANNE TC: ${o.ANNETC} | BABA ADI: ${o.BABAADI} | BABA TC: ${o.BABATC} | UYRUK: ${o.UYRUK}`).join('\n')
                  //  interaction.channel.send(`:tada: ${isim} ${soyisim} İsminde **${data.length}** Kişi Bulundu.`)
                    ...
                    interaction.user.send({ 
                        content: `:tada: ${isim} İsminde **${data.length}** Kişi Bulundu.`, 
                        files: [ dosyahazırla ]
                    })
                    interaction.reply({ content: "Dm üzerinden yolladım :white_check_mark:", ephemeral: false })
                })  
                } else { 
                  con.query(`SELECT * FROM 101m WHERE ADI="${isim}"`, function (err, result) {
                    interaction.reply({ content: "Kişi Aranıyor.", ephemeral: true })
                    if (err) throw err;
              let data = JSON.parse(JSON.stringify(result))
              if(data.length < 1) return interaction.user.send({ content: "Görünüşe Göre Bir Sonuç Bulunamadı Bunun Sebebi Aşağıdaki Maddelerden Biri Olabilir. \n ・ Sorguladığınız Numara Yeni İse Sistemimize Kayıtlı Olmayabilir \n ・ Numarayı Yanlış Girmiş Olabilirsin Numaranın Başında 0 Olmamalı Ve Sayılar Birleşik Olmalıdır", ephemeral: true })
              ...
              let as31 = data.map((o) => `TCSİ: ${o.TC} | ADI: ${o.ADI} | DOGUMTARİHİ: ${o.DOGUMTARIHI} | İL: ${o.NUFUSIL} | İLÇE: ${o.NUFUSILCE} | ANNE ADI: ${o.ANNEADI} | ANNE TC: ${o.ANNETC} | BABA ADI: ${o.BABAADI} | BABA TC: ${o.BABATC} | UYRUK: ${o.UYRUK}`).join('\n')
                   // interaction.channel.send(`:tada: ${isim} İsminde **${data.length}** Kişi Bulundu.`)
                    let dosyahazırla = new Discord.MessageAttachment(Buffer.from(as31), `distorted.txt`);
